...
if (isset($_GET['date']) && preg_match('/^[0-9]{10}$/', $_GET['date'])) {
    ...
    $sql = "SELECT A.empf_anrede, A.empf_vorname, A.empf_nachname, A.empf_firma, A.empf_strasse, A.empf_hausnummer,
            A.empf_plz, A.empf_ort, A.empf_telefon, A.empf_land, A.empf_email, A.gutschein_nummer, B.prae_bezeichnung, B.prae_gewicht, A.einloese_datum, A.wunsch_versand_datum
    ...
    $kommando->bind_result($anrede, $vorname, $nachname, $firma, $strasse, $hnr, $plz, $ort, $telefon, $land, $email, $gsnummer, $praesent, $gewicht, $einloesedatum, $versanddatum);


    // Länder Liste um Ländernamen in ISO code um zuwandeln
    ...
    $laender['de']['SK'] = "Slowakei";
    $laender['en']['SI'] = "Slovenija";
    $laender['de']['SI'] = "Slovenija";
//    $laender['de']['SI'] = "Slowenien";
    ...
    $csv_output = '';
    $csv_output .= 'TNT-Kundennummer' . ";" . 'Name' . "; " . 'Strasse1' . "; " . 'Strasse2' . "; " . 'Kontakt' . "; " . 'PLZ' . ";  " . 'Stadt' . ";  " . 'Telefon' . "; " . 'Land ' . ";" . 'Service' . "; " . 'Option' . ";  " . 'Referenz' . ";  " . 'Gewicht' . "; " . 'Packmittel' . "; " . 'Länge' . ";  " . 'Breite' . ";  " . 'Höhe' . "; " . 'Inhalt' . "; " . 'Anzahl' . "; " . 'Email' . "; " . 'Einlösedatum' . "; " . 'Versanddatum';
    $csv_output .= "\n";
    while ($kommando->fetch()) {

        if ($landCode = array_search($land, $laender['de'])) {
            $landCode = array_search($land, $laender['de']);
        } else {
            $landCode = array_search($land, $laender['en']);
        }

        $csv_output .= '70049012'.";".htmlspecialchars_decode($vorname,ENT_QUOTES).' '.htmlspecialchars_decode($nachname,ENT_QUOTES).";".$strasse.' '.$hnr.";" . ' ' . ";".htmlspecialchars_decode($firma, ENT_QUOTES).";".$plz.";".$ort.";".str_replace(' ', '', $telefon).";".$landCode.";".'12B'.";". ' ' .";".$gsnummer.";".$gewicht.";".'Karton'.";".'10'.";".'10'.";".'10'.";".$praesent.";".'1'.";".$email.";".date('d.m.Y',$einloesedatum).";".date('Ymd', $versanddatum);

        $csv_output .= "\n";
    }

    $filename = "versandliste_erweitert_tnt_mit_Telefon-" . date('d-m-Y', $versanddatum);
    // convert encoding so tnt-mapper can read it.
    //    $csv_output = iconv("UTF-8", "Windows-1252", $csv_output);
    $csv_output = mb_convert_encoding($csv_output, "Windows-1252", "UTF-8");
