...
class tc_calendar{
	...
	function writeScript(){
		...
		if($this->date_picker){
			echo("<div style=\"position: relative; z-index: $this->zindex; float: right; padding-right:15px;\">");
	...
	function eHidden($suffix, $value) {
        ...
        function qHidden($suffix, $value) {
		if($suffix) $suffix = "_".$suffix;
		echo("<input type=\"text\" name=\"".$this->objname.$suffix."\" id=\"".$this->objname.$suffix."\" value=\"".$value."\" class=\"text_field_medium\" />");
	}

	//write hidden components
	function writeHidden(){
		$this->qHidden('', $this->getDate());
