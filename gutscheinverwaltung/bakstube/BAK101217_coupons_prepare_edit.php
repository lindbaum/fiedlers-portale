...
if(isset($_POST['submit'])) {   
    ...
    if(isset($_POST['more']) && $_POST['more'] == 'on') {
        $more = true;
        (isset($_POST['more_anzahl']) && preg_match('/^([0-9]+)$/', $_POST['more_anzahl']) && $_POST['more_anzahl'] > 0) ? $more_anzahl = $_POST['more_anzahl'] : $error .= '<br />Bitte geben Sie eine Anzahl an Gutscheinen an, um die Daten zu &uuml;bernehmen.';
    }
    ...
    if(empty($error)) {
        if($more === true) {
            ...
            if($kommando->error) $error .= '<img src="images/cross.png" align="top" />&nbsp;Beim Speichern der Daten f&uuml; Gutscheinnummer ' . $edit . ' ist ein Fehler aufgetreten!<br />';
            else array_push($stack, $edit);
            ...
            for($x = 1; $x <= $more_anzahl; $x++) {
                ...
                $kommando = $db->prepare($sql);
                $kommando->bind_param('ssiiisi', $versand, $_POST['rechnungsnummer'], $_POST['berechnung'], $_POST['praesent'], $_POST['verschenker'], $timestamp_heute, $new_number);
                $kommando->execute();
                
                if($kommando->error) $error .= '<img src="images/cross.png" align="top" />&nbsp;Beim Speichern der Daten f&uuml; Gutscheinnummer ' . $new_number . ' ist ein Fehler aufgetreten!<br />';
                else array_push($stack, $new_number);
            }
                    ...
                    <form method="post" autocomplete="off">
                        <table width="400" border="0">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <th scope="col" colspan="2" style="height: 40px;"><input type="checkbox" name="more" />&nbsp;&nbsp;Daten f&uuml;r weitere Gutscheine &uuml;bernehmen
                                </tr>
                                <tr>
                                    <th>Anzahl <a class="info" href="#"><img src="images/info.jpg" align="top" /><span>Anzugeben ist die Anzahl der weiteren Gutscheine, nicht die Gesamtanzahl.</span></a></th></th>
                                    <td>
