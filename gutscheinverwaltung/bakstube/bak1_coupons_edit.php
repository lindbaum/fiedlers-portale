...
if(isset($_POST['submit'])) {
    ...
    if(empty($error)) {
        ...
        elseif(isset($redeem) && $redeem == 1) {
            ...
            $kommando = $db->prepare($sql);
            $kommando->bind_param('ssiiisssssssssssii', $befristung, $rechnungsnummer, $berechnung, $praesent, $verschenker, $anrede, $name, $vorname,
                                  $strasse, $hnr, $plz, $ort, $land, $email, $telefon, $versand, $einloesungsort, $edit);
            $kommando->execute();
                    ...
                    <form method="post" autocomplete="off">
                        ...
                        <?php if(isset($redeem) && $redeem == 1) { ?>
                            ...
                            <table width="400" border="0">
                                ...
                                <tbody>
                                    <tr>
                                        <th width="125" scope="col">Anrede</th>
                                        <td>
                                    ...
                                    <tr>
                                        ...
                                        ($aktueller_monat >= 10) ? $end_jahr = ($start_jahr + 1) : $end_jahr = $start_jahr;
                                        
                                        $myCalendar->setYearInterval($start_jahr, $end_jahr);
                                        
                                        // Zeitspanne setzen
                                        // bis 10.00 buchbar am gleichen Tag, dann ab Folgetag!
                                        (date('h',time()) < 10) ? $start_datum = mktime(0,0,0,date('m'),date('d'),date('Y')) : $start_datum = mktime(0,0,0,date('m'),date('d', strtotime('+ 1 day')),date('Y'));
                                        $end_datum = $start_datum + (60 * 60 * 24 * 84); // 84 Tage (12 Wochen) addieren 
