        ...
        <div id="content">
            ...
            if(isset($_POST['date1']) && $_POST['date1'] != '0000-00-00') {
                ...
                $sql = "SELECT A.empf_anrede, A.empf_vorname, A.empf_nachname, A.empf_firma, A.empf_strasse, A.empf_hausnummer,
                        A.empf_plz, A.empf_ort, A.empf_email, A.gutschein_nummer, B.prae_bezeichnung, B.prae_gewicht, A.einloese_datum, A.wunsch_versand_datum
                ...
                $kommando->bind_result($anrede, $vorname, $nachname, $firma, $strasse, $hnr, $plz, $ort, $email, $gsnummer, $praesent, $gewicht, $einloesedatum, $versanddatum);
                
                new_tbl('100%', 1);
                
                $tbl_header = array('Anrede', 'Vorname', 'Name', 'Firma', 'Strasse', 'Nr.', 'PLZ', 'Ort', 'Email', 'GS-Nr.', 'Pr&auml;sent', 'Gewicht', 'Einl.-Datum', 'Vers.-Datum');
                ...
                while($kommando->fetch()) {
                    printf('<tr><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td></tr>',
                           $anrede, $vorname, $nachname, $firma, $strasse, $hnr, $plz, $ort, $email, $gsnummer, $praesent, $gewicht, date('d.m.Y', $einloesedatum), date('d.m.Y', $versanddatum));
                }
                ...
                <tfoot>
                    <tr>
                        <td colspan="14"><?php printf('%s Eintr&auml;ge gefunden.', $kommando->num_rows); ?></td>
                    </tr>
                </tfoot>
